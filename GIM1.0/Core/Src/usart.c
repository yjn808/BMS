...
// GNSS相关
extern char gnss_rx_buffer[512];         
extern volatile _Bool gnss_data_ready;    
extern uint16_t gnss_rx_index;

// MAVLink相关
extern UART_HandleTypeDef *huart_mavlink;
extern uint8_t uart_rx_byte;
extern volatile uint8_t uart_rx_buffer[512];
...
void HAL_UART_MspInit(UART_HandleTypeDef* uartHandle)
{
  ...
  */
    ...
    PD6     ------> USART2_RX
    */
    GPIO_InitStruct.Pin = GPIO_PIN_5|Moving_DE_Pin|GPIO_PIN_6;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void HAL_UART_MspDeInit(UART_HandleTypeDef* uartHandle)
{
  ...
  /* USER CODE END USART2_MspDeInit 0 */
    ...
    PD6     ------> USART2_RX
    */
    HAL_GPIO_DeInit(GPIOD, GPIO_PIN_5|Moving_DE_Pin|GPIO_PIN_6);
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
  ...
  {
    if (Rxdata[0] == 0x0F && Rxdata[24] == 0x00 && Size == 25)
    {
      Sbus_Parse(Rxdata, ch);
      sbus_data_ready = 1;
    }
...
// // UART接收完成中断回调函数（用于GNSS数据接收）
// void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
//     if (huart->Instance == USART1) {
//         char received_char = gnss_rx_buffer[gnss_rx_index];
        
//         // 检查是否为换行符，表示一行数据结束
//         if (received_char == '\n') {
//             if (gnss_rx_index > 0) {
//                 // 确保字符串正确终止
//                 if (gnss_rx_index < sizeof(gnss_rx_buffer) - 1) {
//                     gnss_rx_buffer[gnss_rx_index] = '\0';
//                 }
//                 gnss_data_ready = true;
//                 return; // 数据准备好了，不重新启动接收
//             }
//         } 
//         // 忽略回车符，继续接收其他字符
...
//         // 继续接收下一个字符
//         HAL_UART_Receive_IT(&huart1, (uint8_t*)&gnss_rx_buffer[gnss_rx_index], 1);
//     }
// }
