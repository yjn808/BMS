...
// 函数声明
void MPU_ConfigRegion(uint8_t Region, uint8_t DisableExec, uint8_t AccessPermission,
                      uint8_t TypeExtField, uint32_t Address, uint32_t Size,
                      uint8_t IsShareable, uint8_t IsCacheable, uint8_t IsBufferable);
void MPU6050_Config(uint8_t Region, uint8_t Mode, uint32_t Address, uint32_t Size);
