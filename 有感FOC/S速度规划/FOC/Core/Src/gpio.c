...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    static float target_speed = 0.0f;  // 当前目标速度
    ...
    switch(GPIO_Pin)
    {
        ...
        case GPIO_PIN_10:       // PC10 - Button1 (Motor Start/Stop)
            if(target_speed != 0)
            {
                target_speed = 0.0f;  // 停止电机
                Motor_SCurve_SetTarget(target_speed); 
            }
            else
            {
                target_speed = 50.0f;  // 启动电机，50 rad/s
                Motor_SCurve_SetTarget(target_speed);
            }
            break;
            
        case GPIO_PIN_11:       // PC11 - Button2 (改变转向)
            target_speed = -target_speed;  // 改变转向
            Motor_SCurve_SetTarget(target_speed);
            break;
            
        case GPIO_PIN_13:       // PC13 - Button3 (速度增加/减少)
            // 如果当前是正转或停止，增加速度
            if(target_speed >= 0)
                target_speed += 20.0f;
            else
                target_speed -= 20.0f;  // 反转时，数值减少（更负）
