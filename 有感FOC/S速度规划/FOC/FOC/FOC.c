...
{  
    ...
    Motor_SCurve_SetParams(20.0f, 80.0f);  
    Motor_SCurve_SetTarget(0.0f);           // 初始目标速度为0
rtX.Id_ref = 0.0f;      // d轴电流为0
rtX.Iq_ref = 0.0f;      // q轴电流由速度环控制，不要手动设置！
// 注释掉：rtX.Speed_ref = 100;  // 由S曲线控制
rtX.Angle_ref = 0.0f;
...
void Motor_Run(void)
{
    ...
    Encoder.angle_mach = (float)TIM4->CNT/4000*2*PI;
