...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    switch(GPIO_Pin)
    {
        ...
        case GPIO_PIN_10:       // PC10 - Button1 (Motor Start/Stop)
            ...
            {
                rtX.Speed_ref = 100.0f;  // 启动电机
                printf("Motor Start\r\n");
            }
            break;
            
        case GPIO_PIN_11:       // PC11 - Button2 (Motor CW/CCW)
            rtX.Speed_ref = -rtX.Speed_ref;  // 改变转向
            printf("Direction changed, Speed: %f\r\n", rtX.Speed_ref);
            break;
            
        case GPIO_PIN_13:       // PC13 - Button3 (Speed increase)
            if(rtX.Speed_ref >= 0)
                rtX.Speed_ref += 10.0f;
            else
                rtX.Speed_ref -= 10.0f;
            ...
            // 限制最大速度
            if(rtX.Speed_ref > 500.0f) rtX.Speed_ref = 500.0f;
            if(rtX.Speed_ref < -500.0f) rtX.Speed_ref = -500.0f;
