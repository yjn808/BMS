...
{
	float speed_err,speed_ramp;
	
	speed_err = rtX.Speed_ref - Encoder.speed_filter;
	if(fabs(speed_err) > MAX_SPEEDSTEP)
	{
		if(speed_err >= 0)
			speed_ramp = Encoder.speed_filter + MAX_SPEEDSTEP;
		else 
			speed_ramp = Encoder.speed_filter - MAX_SPEEDSTEP;
	}else
		speed_ramp = rtX.Speed_ref;
	
	/* 速度环 */
	if(Encoder.flag == 1)
		Speed_PID_Calc(speed_ramp,Encoder.speed_filter,&rtX.Iq_ref,&Speed_PID);
}
